...
def show_feature_map(feature_map,layer,name='rgb',rgb=False, en=255):
    ...
    if rgb:
        ...
        cv2.imwrite('data/'+layer+'/'+name+".png",feature_map*en)
        #plt.show()
    else:
        #plt.figure()
        for index in range(1, feature_map_num+1):
            t = (feature_map[index-1]*en).astype(np.uint8)
            #t = cv2.applyColorMap(t, cv2.COLORMAP_HOT)
...
def calc_psnr(sr, hr, dataset=None, scale=1, rgb_range=1):
    ...
    return 20*torch.log10(torch.max(hr)-torch.min(hr))-10*torch.log10(mse)
    # print(mse)
