...
class SRNO(nn.Module):
    def __init__(self, encoder_spec, width=256, blocks=16, out_dim = 3):
        ...
        self.fc2 = nn.Conv2d(256, out_dim, 1)
    ...
    def query_rgb(self, coord, cell):      
        ...
        # feats = feat_s[0]+feat_s[1]+feat_s[2]+feat_s[3]
        # grid = torch.cat([coord.permute(0,3,1,2),coord.permute(0,3,1,2),coord.permute(0,3,1,2),coord.permute(0,3,1,2),feats,feats,feats,feats, \
        #     rel_cell.unsqueeze(-1).unsqueeze(-1).repeat(1,1,coord.shape[1],coord.shape[2])],dim=1)
